var app = (function () {
	...
	function initModels() {
		...
		createModel("plane", "wireframe", [0, -.8, 0], [0, 0, 0],
			[1, 1, 1]);
		createModel("sphere", fs, [0, -0.5, 0], [0, 0, 0],
			[.7, .7, .7]);
		createModel("sphere", fs, [0, 0.5, 0], [0, 0, 0],
			[.5, .5, .5]);
		createModel("sphere", fs, [0, 1.3, 0], [0, 0, 0],
			[.4, .4, .4]);
		createModel("oktaeder", fs, [0, 0.5, 1], [0, 0, 0],
			[.4, .4, .4]);
		...
		window.onkeydown = function (evt) {
			...
			switch (key) {
				case 39:
					...
					break;
				case 37:
					camera.zAngle -= sign * deltaRotate; // -5° in Radiant
					break;
